...
setwd("e:/08.SAGPEK//sangerseqR_1.36.0/sangerseqR/R/")

abi_file2 <- "230619_0016.J1.F_A01.ab1"


obj<- read.abif(abi_file2)


test_sangerseq <- sangerseq(obj)



test_basecall<- makeBaseCalls(test_sangerseq)

peakAmp <- peakAmpMatrix(test_basecall)
...
write.table(peakAmp, file="J1.test.peakAMP.txt",row.names=F,quote=F,sep="\t")
