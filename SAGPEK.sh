 	 ...
 	 case $OPT in
		 t)type="$OPTARG"
			...
			if [ "$type" = 'custom' ] || [ "$type" =  'cattle_DUMPS' ] || [ "$type" = 'cattle_CN' ] || [ "$type" =  'TEST' ] || [ "$type" = 'HBV' ] || [ "$type" = 'PAH' ]; then 
				echo "The type of ABI files is: $type"
			else 
				echo "Invalid option: $type! Please refer Usages with -u"
				exit 1
			fi
			;;
		 ...
		 g)tag="$OPTARG"
			 parameter_count=1
			 echo "The customized tag file is: $tag"
			 ;;
		 ...
		 ?)		
			 parameter_count=1
			echo "Invalid option or Parameters not provided, please see the Help using -h or the -u option:" >&2
			exit 1
		  ;;
		 
		 :) 
		  echo "Argument missing from -%s option\n" $OPTARG 
		  exit 1
		  ;;
...
if [ "$type" = 'custom' ]; then
	if [[ -z $tag   ]]; then
		echo "As the type is custom, the customized tag file must be provided!"
		exit 1
	fi
fi
...
if ! type "Rscript" > /dev/null; then
	echo "R software and Rscript not installed or activated!"
	exit 1
fi
